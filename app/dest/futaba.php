...
define("TITLE", 'Grillout');		//タイトル（<title>とTOP）
...
function form(&$dat,$resno,$admin=""){
  ...
  if($resno){
    ...
    $msg .= "<font color=#FFFFFF>Simple</font>\n";
    $msg .= "</th></tr></table>\n";
  }
  if($admin){
...
';
  ...
  <tr><td bgcolor=#ee8888><b>Name</b></td><td><input type=text name=name size="28"></td></tr>
  <tr><td bgcolor=#ee8888><b>E-mail</b></td><td><input type=text name=email size="28"></td></tr>
  <tr><td bgcolor=#ee8888><b>Post Title</b></td><td><input type=text name=sub size="35">
  <input type=submit value="Submit"></td></tr>
  <tr><td bgcolor=#ee8888><b>Comment</b></td><td><textarea name=com cols="48" rows="4" wrap=soft></textarea></td></tr>
  ';

  if(RESIMG || !$resno){
    $dat.='<tr><td bgcolor=#ee8888><b>Upload Image</b></td>
    <td><input type=file name=upfile size="35">
    [<label><input type=checkbox name=textonly value=on>Post Without Image?</label>]</td></tr>';
  }

  $dat.='<tr><td bgcolor=#ee8888><b>Deletion Key</b></td><td><input type=password name=pwd size=8 maxlength=8 value=""><small>(記事の削除用。英数字で8文字以内)</small></td></tr>
  <tr><td colspan=2>
  <small>
  <LI>Attachable files: GIF, JPG, PNG Depending on the browser, it may not be possible to attach properly.
  <LI>Maximum upload size is '.MAX_KB.' KB
  <LI>Note that the image will be scaled down if it exceeds a width of '.MAX_W.' px or a height of '.MAX_H.' px.
  ...
  for($i = $st; $i < $st+PAGE_DEF; $i++){
    ...
    if($ext && is_file($img)){
      ...
      if($w && $h){//サイズがある時
        if(@is_file(THUMB_DIR.$time.'s.jpg')){
          $imgsrc = "<small>Showing thumbnail. Click to see original.</small><br><a href=\"".$src."\" target=_blank><img src=".THUMB_DIR.$time.'s.jpg'.
      ...
      $dat.="Image Title:<a href=\"$src\" target=_blank>$time$ext</a>-($size B)<br>$imgsrc";
    }
    ...
    $dat.="\n<blockquote>$com</blockquote>";
     ...
     if($lineindex[$no]-1 >= LOG_MAX*0.95){
      $dat.="<font color=\"#f00000\"><b>This is an old thread, and it might disappear soon.</b></font><br>\n";
     }
    ...
    if(!$resno){
      ...
      elseif($s>1){
       $dat.="<font color=\"#707070\">Replies".
              ($s - 1)."Omitted. Press the reply button to read all.</font><br>\n";
      }
    }
    else{
      $s=1;
    ...
    for($k = $s; $k < count($treeline); $k++){
      ...
      if($ext && is_file($img)){
        ...
        if($w && $h){//サイズがある時
          if(@is_file(THUMB_DIR.$time.'s.jpg')){
            $imgsrc = "<small>Thumbnail</small><br><a href=\"".$src."\" target=_blank><img src=".THUMB_DIR.$time.'s.jpg'.
        " border=0 align=left width=$w height=$h hspace=20 alt=\"".$size." B\"></a>";
          }
          else{
            $imgsrc = "<a href=\"".$src."\" target=_blank><img src=".$src.
...
<input type=hidden name=mode value=usrdel>【Delete Post】[<input type=checkbox name=onlyimgdel value=on>Only Erase Image?]<br>
Deletion Key<input type=password name=pwd size=8 maxlength=8 value="">
<input type=submit value="Confirm Deletion"></form></td></tr></table>';
...
function foot(&$dat){
  $dat.='
<center>
<small><!-- Futaba Ripoff by ijustwannagrill -->
</small>
</center>
</body></html>';
}
...
function regist($name,$email,$sub,$comment,$url,$pwd,$upfile,$upfile_name,$resto){
  ...
  if($upfile&&file_exists($upfile)){
    ...
    if(!is_file($dest)){
      error("Your upload failed. The wacky server stuff might not support it.",$dest);
    }
    $size = getimagesize($dest);
    if(!is_array($size)){
      error("Sorry, you can only upload images.",$dest);
    }
    $is_uploaded = ImageFile::getNew()->isUploaded($badfile, $dest);
    if ($is_uploaded === true) {
      error("Sorry, there's that same image on here already. Try naming it something else.", $dest); //拒絶画像
      return;
    }
    chmod($dest,0666);
    ...
    $mes = "Image $upfile_name Upload Successful!<br><br>";
  }
  ...
  if($_SERVER["REQUEST_METHOD"] != "POST"){
    error("No messing around.(post)",$dest);
  }
  // フォーム内容をチェック
  ...
  if(!$resto&&!$textonly&&!is_file($dest)){
    error("There's no image.",$dest);
  }
  if(!$comment&&!is_file($dest)){
    error("You need to write something.",$dest);
  }

  $name=preg_replace("/Moderation/","\"Moderation\"",$name);
  $name=preg_replace("/Deletion/","\"Deletion\"",$name);

  if(strlen($comment) > 1000){
    error("Oof, your message is too long.",$dest);
  }
  if(strlen($name) > 100){
    error("Oof, your message is too long.",$dest);
  }
  ...
  if(strlen($resto) > 10){
    error("Something's not right here...",$dest);
  }
  if(strlen($url) > 10){
    error("Something's not right here...",$dest);
  }
  ...
  $youbi = array('Day','Month','Fire','Water','Wood','Money','Earth');
  ...
  for($i=0;$i<$imax;$i++){
    ...
    if(RENZOKU && $p && $time - $ltime < RENZOKU){
      error("You're posting too frequently. Please try again in a little bit.",$dest);
    }

    if(RENZOKU && $p && $time - $ltime < RENZOKU2 && $upfile_name){
      error("You're uploading images too frequently. Please try again in a little bit.",$dest);
    }
  ...
  if($dest&&file_exists($dest)){
    ...
    for($i=0;$i<$imax;$i++){ //画像重複チェック
      ...
      if($p==$is_uploaded&&file_exists($path.$timep.$extensionp)){
        error("Sorry, there's that same image on here already. Try naming it something else.",$dest);
      }
    }
  }
  ...
  echo "<body>$mes Switch Screens</body></html>";
}
?>
...
function usrdel($no,$pwd){
  ...
  if(!$flag){
    error("Either the Deletion Key is incorrect, or the Post has already been deleted.");
  }
}
?>
...
function valid($pass){
  ...
  echo "<font color=#FFFFFF>Janny Mode</font>\n";
  echo "</th></tr></table>\n";
  echo "<p><form action=\"".PHP_SELF."\" method=POST>\n";

  // ログインフォーム
  if(!$pass){
    echo "<center><input type=radio name=admin value=del checked>Post Deletion";
    echo "<input type=radio name=admin value=post>Posted by Admin<p>";
...
function admindel($pass){
  ...
  echo "<center><P>Check the checkbox of the article you want to delete, and click the delete button.\n";
  echo "<p><input type=submit value=\"Delete\">";
  echo "<input type=reset value=\"Reset\">";
  echo "[<input type=checkbox name=onlyimgdel value=on>Only Delete Image]";
  echo "<P><table border=1 cellspacing=0>\n";
  echo "<tr bgcolor=6080f6><th>Delete</th><th>PostNo</th><th>Date</th><th>Title</th>";
  echo "<th>Contributor</th><th>Reply</th><th>Name</th><th>Attachment<br>(Bytes)</th><th>md5</th>";
  echo "</tr>\n";
  $line = file(LOGFILE);
  ...
  echo "</table><p><input type=submit value=\"Confirm Deletion$msg\">";
  echo "<input type=reset value=\"リセット\"></form>";

  $all = (int)($all / 1024);
  echo "【 Image Size: <b>$all</b> KB 】";
...
[<a href="'.PHP_SELF.'?mode=admin">Moderation</a>]
<p align=center>
...
function init(){
  ...
  if(!is_writable(realpath("./"))){
    error("Cannot write to current directory.<br>");
  }
  ...
  foreach($chkfile as $value){
    if(!file_exists(realpath($value))){
      ...
      set_file_buffer($fp, 0);
      if($value==LOGFILE){
        fputs($fp,"1,2002/01/01(Month) 00:00,Anonymous,,Untitled,No text,,,,,,,,\n");
      }
      if($value==TREEFILE){
        fputs($fp,"1\n");
      }
    ...
    if(!is_writable(realpath($value))){
      $err.=$value."Cannot write<br>";
    }
    if(!is_readable(realpath($value))){
      $err.=$value."Cannot read<br>";
    }
  }
  @mkdir(IMG_DIR,0777);
  @chmod(IMG_DIR,0777);
  if(!is_dir(realpath(IMG_DIR))){
    $err.=IMG_DIR."Nonexistent<br>";
  }
  if(!is_writable(realpath(IMG_DIR))){
    $err.=IMG_DIR."Cannot write<br>";
  }
  if(!is_readable(realpath(IMG_DIR))){
    $err.=IMG_DIR."Cannot read<br>";
  }
  if(USE_THUMB){
    @mkdir(THUMB_DIR,0777);
    @chmod(THUMB_DIR,0777);
    if(!is_dir(realpath(IMG_DIR))){
      $err.=THUMB_DIR."Nonexistent<br>";
    }
    if(!is_writable(realpath(THUMB_DIR))){
      $err.=THUMB_DIR."Cannot write<br>";
    }
    if(!is_readable(realpath(THUMB_DIR))){
      $err.=THUMB_DIR."Cannot read<br>";
    }
  }
  if($err){
    error($err);
